...
print(iris1) # can also use glipse(iris1)
```
...
print(iris2) #can also use glimpse(iris2)
```
...
#answer key doesn't have by= but I guess both does the job
```
...
```{r}
iris5 <- group_by(iris4, Species) #. <- group_by(iris4, Species) 
